...
## Purpose
Milestone 2 should introduce household scope and the first meaningful household trade-off. Its job is to make "ours" real between personal and civic life without resetting or casually rewriting the verified Milestone 1 baseline. This doc remains the scope and boundary anchor for Milestone 2; the verified pantry-plus-reserve household slice now exists, but this plan still does not authorize silent widening beyond that slice.
